...
extern crate proc_macro; // gain access to compiler tokens
extern crate syn;
#[macro_use]
extern crate quote;

//taken from https://doc.rust-lang.org/1.30.0/book/second-edition/appendix-04-macros.html

#[proc_macro_derive(VertexAttribPointers, attributes(location))] //use location as an atribute
...
fn generate_struct_field_vertex_attrib_pointer_call(field: &syn::Field) -> quote::Tokens {
    ...
    //iterated through attributes to find location 
    let location_attr = field.attrs.iter().filter(|a| a.value.name() == "location").next().unwrap_or_else(|| panic!("Field {:?} is missing #[location = ?] attribute", field_name));
